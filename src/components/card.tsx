...
export const Card = ({ movie, handdleModal }: { movie: Result, handdleModal: () => void }) => {


  const handdleModalId = () => {
    localStorage.setItem("id_movie", JSON.stringify(movie.id));
    setTimeout(() => {
      handdleModal();
    }, 20);
  }
  ...
  return (
    <>
      ...
      {movie &&
        <div className='group inline-block min-w-fit rounded-md  overflow-visible
        transition-all ease-out duration-400'>
          <div className="w-[242px] group-hover:relative  group-hover:flex  group-hover:w-[242px] transition-all ease-in duration-400"></div>
          ...
          " >
            ...
            <div id='body_card' className="hidden group-hover:block text-white px-1 text-[9px] delay-500">
