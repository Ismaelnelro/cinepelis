        ...
        <ul className='flex lg:pl-9 md:pl-3 lg:gap-4  md:gap-2 w-full h-fit'>
          {
            LINK_NAVBAR.map((link, i) => (
              <li className='list-none px-1 md:text-xs lg:text-sm'>
                <Link to={`${i}`} key={i} className='focus:text-white focus:font-medium hover:text-zinc-400 transition-all ease-in duration-300 h-auto  max-w-fit tracking-tight '>
                  {link}
                </Link>
              </li>
            ))
